...
exports.getPayoutsHistory = exports.getFinancialOverview = exports.getEnterpriseReport = void 0;
...
const toBRL = (value) => value.toLocaleString("pt-BR", { style: "currency", currency: "BRL" });
/* -------------------------------------------------------
...
const getEnterpriseReport = async (req, res) => {
    try {
        ...
        const payload = token ? await (0, auth_1.decodeToken)(token) : null;
        ...
        if (!isAdmin)
            filter.userId = new mongoose_1.default.Types.ObjectId(payload?.id);
        else if (userId)
            filter.userId = new mongoose_1.default.Types.ObjectId(userId);
        const transactions = await transaction_model_1.Transaction.find(filter).lean();
        const policies = await retentionPolicy_model_1.RetentionPolicy.find().lean();
        ...
        for (const policy of policies) {
            ...
            totalRetention += methodTx.reduce((acc, t) => acc + ((t.netAmount || 0) * (policy.percentage / 100)), 0);
        }
        const totalAvailable = round(totalNet - totalRetention);
        const totalByMethod = {};
        const totalByStatus = {};
        for (const t of transactions) {
            totalByMethod[t.method] = toBRL((totalByMethod[t.method]
        ...
        const topProducts = await transaction_model_1.Transaction.aggregate([
            { $match: filter },
        ...
        const topSellers = await transaction_model_1.Transaction.aggregate([
            { $match: filter },
            {
                $group: {
                    _id: "$userId",
...
exports.getEnterpriseReport = getEnterpriseReport;
/* -------------------------------------------------------
💰 2. Visão Financeira – Saldo atual, retenções, projeções
-------------------------------------------------------- */
const getFinancialOverview = async (_req, res) => {
    try {
        const totalWallets = await wallet_model_1.Wallet.aggregate([
            {
                $group: {
                    _id: null,
...
-------------------------------------------------------- */
const getPayoutsHistory = async (_req, res) => {
    try {
        const payouts = await wallet_model_1.Wallet.aggregate([
            { $unwind: "$log" },
...
exports.getPayoutsHistory = getPayoutsHistory;
